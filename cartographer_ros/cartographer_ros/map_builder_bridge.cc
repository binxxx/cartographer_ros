...
std::shared_ptr<octomap::OcTree> MapBuilderBridge::GetMatchingOctomap() {
  cartographer::mapping::TrajectoryBuilder* trajectory_builder =
        map_builder_.GetTrajectoryBuilder(0);
  // std::cout << "??\n" << std::endl;
  if (trajectory_builder) {
    return trajectory_builder->GetMatchingOctomap();
  } else {
    LOG(WARNING) << "MapBuilderBridge::GetMatchingOctomap()::Returning nullptr.";
    return nullptr;
  }
}

std::shared_ptr<cartographer::mapping::Submap> MapBuilderBridge::GetMatchingSubmap() {
  cartographer::mapping::TrajectoryBuilder* trajectory_builder =
        map_builder_.GetTrajectoryBuilder(0);
  if (trajectory_builder) {
    return trajectory_builder->GetMatchingSubmap();
  } else {
    LOG(WARNING) << "MapBuilderBridge::GetMatchingOctomap()::Returning nullptr.";
    return nullptr;
  }
...
cartographer::transform::Rigid3d MapBuilderBridge::GetMatchingLocalPose() {
  ...
  for (size_t trajectory_id = 0; trajectory_id < all_submap_data.size();
    ...
    //      submap_index < all_submap_data[trajectory_id].size(); ++submap_index) {
      int submap_index = 0;
      if (all_submap_data[trajectory_id].size() < 2) {
        submap_index = 0;
      } else {
        submap_index = all_submap_data[trajectory_id].size()-2;
      }
      const auto& submap_data = all_submap_data[trajectory_id][submap_index];
      if (submap_data.submap == nullptr) {
        continue;
      ...
      return submap_data.pose;

    // }
  }

  return cartographer::transform::Rigid3d::Identity();
}

cartographer::mapping::PoseEstimate MapBuilderBridge::pose_estimate() {
  cartographer::mapping::TrajectoryBuilder* trajectory_builder =
        map_builder_.GetTrajectoryBuilder(0);
  if (trajectory_builder) {
    return trajectory_builder->pose_estimate();
  } else {
    return cartographer::mapping::PoseEstimate();
  }
}
