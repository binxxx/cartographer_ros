...
std::unique_ptr<::cartographer::sensor::ImageData> 
SensorBridge::ToImageData(const sensor_msgs::Image::ConstPtr& msg) {

  const carto::common::Time time = FromRos(msg->header.stamp);
  return ::cartographer::common::make_unique<
      ::cartographer::sensor::ImageData>(
        ::cartographer::sensor::ImageData{
          time, msg->width, msg->height, msg->encoding, msg->data});
}

void SensorBridge::HandleImageMessage(
  const string& sensor_id, const sensor_msgs::Image::ConstPtr& msg) {
  // std::cout << "SensorBridge::HandleImageMessage()" << std::endl;
  std::unique_ptr<::cartographer::sensor::ImageData> image_data = 
      ToImageData(msg);
  // if (image_data != nullptr) {
  //   trajectory_builder_->AddImageData(sensor_id, image_data->time,
  //                                     image_data->pose);
  // }
}
...
void SensorBridge::HandleImuMessage(const string& sensor_id,
  ...
  // std::cout << "SensorBridge::HandleImuMessage()" << std::endl;
  if (imu_data != nullptr) {
